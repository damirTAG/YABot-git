...
@dataclass
class metadata(data):
    metadata: dict[str, int | float] = field(default_factory=dict)

    @property
    def height(self) -> int | None:
        return self.metadata.get("height")  # type: ignore

    @property
    def width(self) -> int | None:
        return self.metadata.get("width")  # type: ignore
...
class TikTok:
    ...
    async def _makerequest(self, endpoint: str, params: dict) -> dict:
        async with self.session.get(
            os.path.join(self.host, endpoint), params=params, headers=self.headers
        ) as response:
    ...
    async def _download_file(self, url: str, path: str):
        async with self.session.get(url) as response:
            ...
            with (
                ...
                tqdm(unit="B", unit_scale=True, desc=os.path.basename(path)) as pbar,
            ):
    ...
    async def image(self, download_dir: str | None = None):
    ...
    async def video(self, video_filename: str | None = None, hd: bool = False):
        ...
        async with self.session.get(video_url) as response:
            ...
            with (
                open(video_filename, "wb") as file,
                tqdm(total=total_size, unit="B", unit_scale=True, desc=video_filename) as pbar,
            ):
        ...
        return metadata(
            ...
            type="video",
            metadata={"duration": duration, "width": width, "height": height},
        )
    ...
    async def search(
        self, method: Literal["keyword", "hashtag"], keyword: str, count: int = 10, cursor: int = 0
    ) -> list:
        """
    ...
    async def download_sound(
        self, link: str, audio_filename: str | None = None, audio_ext: str = ".mp3"
    ) -> str:
    ...
    async def download(self, link: str, video_filename: str | None = None, hd: bool = False) -> str:
        """
