...
class YandexMusicSDK:
    ...
    @staticmethod
    def _extract_album_id(url: str) -> int | None:
    ...
    async def _request(self, url, params: dict = None, method: str = "GET", headers: dict = None):
        """
    ...
    def insert_metadata(self, metadata: TrackData):
        """
        Note:
        ...
        if filename and os.path.exists(filename):
            ...
            self.logger.warning(
                "Sorry, the metadata inserting failed, seems like the track not downloaded"
            )

    def _prepare_album_metadata(self, album: Album, tracks: list[TrackData] = None) -> AlbumData:
        """
    ...
    def _prepare_filename(id: int):
        return f"track_{id}.{CODEC}" if id else None

    async def _extract_track_id(self, track: str | int) -> str | None:
    ...
    async def _get_lyrics(self, track: Track) -> str | None:
    ...
    async def _download(
        self, dwnld_info: list[DownloadInfo], filename: str | None, upload_dir: str | None = None
    ):
        self.logger.info(
    ...
    async def search(
        ...
        try:
            ...
            async def process_track(track: Track):
                ...
                download_task = asyncio.create_task(track.get_download_info_async(True))
                ...
                if download:
                    await self._download(
                        download_info, self._prepare_filename(track.id), upload_dir
                    )
    ...
    async def get_track(
        ...
        try:
            ...
            self.logger.info(
                f"Fetched track: {track.title} | {self._convert_to_seconds(track.duration_ms)} sec"
            )
            ...
            if download:
                await self._download(download_info, self._prepare_filename(track_id), upload_dir)

            # Prepare and return metadata
            return self._prepare_metadata(
                track, album, lyrics_text=lyrics_text, download_info=download_info
            )
        ...
        if isinstance(album, str):
            if re.match(ALBUM_PATTERN, album):
                ...
                try:
                    ...
                    raise ValueError("Invalid album ID format") from None
        else:
            album_id = album
        ...
        if album_info.volumes:
            ...
            tracks_data = [
                self._prepare_metadata(
                    track, album_info, download_info=await track.get_download_info_async(True)
                )
    ...
    async def get_chart(
        ...
        **kwargs,
        ...
        tracks_short: list[TrackShort] = chart.chart.tracks

        # Process tracks
        processed_tracks: list[TrackData] = []
        ...
        for idx, track_short in enumerate(tracks_short):
            ...
            position_icon = {"down": "🔻", "up": "🔺", "new": "🆕"}.get(chart_info.progress, "▪️")

            if chart_info.position == 1:
                position_icon = "👑"

            self.logger.info(
                f"{position_icon} #{chart_info.position} {track_data.title} - {track_data.artists}"
            )
        ...
        download_summary = (
            f" and downloaded {count if count else len(processed_tracks)} tracks"
            if download
            else ""
        )
        self.logger.info(
...
async def main():
    async with YandexMusicSDK() as ymsdk:
        ...
        track = await ymsdk.get_track(
            "https://music.yandex.kz/album/31926326/track/127718866", download=False, lyrics=False
        )
        first_end = time.time()
