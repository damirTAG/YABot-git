    ...
    def _get_base_options(self) -> dict[str, Any]:
        ...
        options = {
            ...
            "outtmpl": os.path.join(
                self.output_dir, "Greetings from @damirtag yt id %(id)s.%(ext)s"
            ),
            "noplaylist": True,
    ...
    async def download(self, url: str, audio_only: bool = False, duration_limit: int = 360) -> str:
        ...
        try:
            ...
            if duration and duration >= duration_limit:
                self.logger.warning(
                    f"Video duration {duration}s exceeds limit {duration_limit}s. Skipping."
                )
            ...
            if "entries" in info:  # It's a playlist
                ...
                downloaded_file = os.path.join(
                    self.output_dir,
                    f"{info['entries'][0]['title']}.mp4"
                    if not audio_only
                    else f"{info['entries'][0]['title']}.mp3",
                )
            else:  # It's a single video
                if audio_only:
                    downloaded_file = os.path.join(
                        self.output_dir, f"Greetings from @damirtag yt id {info['id']}.mp3"
                    )
                else:
                    downloaded_file = os.path.join(
                        self.output_dir, f"Greetings from @damirtag yt id {info['id']}.mp4"
                    )
            ...
            if "entries" in info:
                for entry in info["entries"]:
                    ...
                    metadata = VideoMetadata(
                        ...
                        # Don't extract download URLs here for performance reasons
                        # They'll be generated when actually downloading
                    )
